...
// Tiny serverless proxy to Ticketmaster so the browser doesn’t get blocked by CORS/origin rules.
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!TM_API_KEY) {
      console.error("[local-events] TM_API_KEY is not set");
      return {
        statusCode: 500,
        headers: CORS_HEADERS,
        body: JSON.stringify({ error: "Server not configured (missing TM_API_KEY)" }),
      };
    }
    ...
    console.log("[local-events] Ticketmaster URL:", tmUrl.toString());

    const tmRes = await fetch(tmUrl.toString());
    if (!tmRes.ok) {
      const text = await tmRes.text();
      console.error("[local-events] Ticketmaster error:", tmRes.status, text);
      return {
        statusCode: 502,
        headers: CORS_HEADERS,
        body: JSON.stringify({ error: "Ticketmaster API error", status: tmRes.status }),
      };
    }
